...
function Result() {
  ...
  useEffect(() => {
    ...
    document.title = val + ' - Google search'

    instance.get(`v1?key=${KEY}&cx=${CX}&q=${val}`).then(res => {
      ...
      console.log(err.response.data.error.details[0].reason)
      const { reason } = err.response.data.error.details[0]
      setError(reason === 'RATE_LIMIT_EXCEEDED' ? 'You have exceeded your search limit. Please try again later.' : reason)
    })
  ...
  return (
    <>
      ...
      <div className="top-section w-full flex justify-between  items-center px-5 pt-3">
        ...
        <div className="ml-5 mt-3 googleIcon w-2/6 lg:w-1/6 md:w-1/6">
          <img src='/assets/google_logo.png' className='object-cover' alt="" />
        </div>
...

        {
          item.length > 0 ? item.map((item, index) => <Element item={item} key={index} />) : (
            ...
            error !== '' ?
              ...
              <span className="text-center text-red-500 text-lg">{error}</span>
